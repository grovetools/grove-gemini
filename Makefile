...
.PHONY: all build test clean fmt fmt-check vet lint run check dev build-all generate-docs schema help
...
# Run E2E tests. Depends on the main 'grove-gemini' binary and the test runner.
# Pass arguments via ARGS, e.g., make test-e2e ARGS="run -i"
test-e2e: build
	@echo "Running E2E tests..."
	@go build -o $(BIN_DIR)/$(E2E_BINARY_NAME) ./tests/e2e/
	@GROVE_GEMINI_BINARY=$(abspath $(BIN_DIR)/$(BINARY_NAME)) tend run -p $(ARGS)
...
help:
	...
	@echo "  make test-e2e ARGS=...- Run E2E tests (e.g., ARGS=\"run -i grove-gemini-basic-generation\")"